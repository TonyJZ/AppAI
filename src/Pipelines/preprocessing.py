...
def add_first_derivative(df, value_column_name, time_column_name):
    # if {time_column_name}.issubset(df.columns):
    #     df.sort_values(by=[time_column_name])
    #     df.reset_index()
    #     pass
    # else :
    ...
    return pd.Series(d1list, index=df.index)
    # print(type(d1))
    # print(d1)
    # return df
...
def add_elapsed_time(data, column_name, starttime = None):
    sorted_data = data.sort_values(by=[column_name])
    if isinstance(sorted_data.iloc[0,:][column_name], dt.datetime):
        first_date = sorted_data.iloc[0,:][column_name]
        return sorted_data.apply(lambda row : (row[column_name] - first_date ).total_seconds(), axis =1)
    else :
        first_date = dateutil.parser.parse(sorted_data.iloc[0,:][column_name])
        return sorted_data.apply(lambda row : (dateutil.parser.parse(row[column_name]) - first_date ).total_seconds(), axis =1)
    # return sorted_data
